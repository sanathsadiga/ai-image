...
def generate_background(prompt: str, reference_data: Optional[bytes] = None, filename: str = "reference.png", format_id: str = "", direction_id: str = "") -> Optional[str]:
    ...
    if format_id == "full-page":
        ...
        size = "1024x1536"
        layout = " OUTPUT ONLY THE 32.9 cm WIDE × 45 cm HIGH NEWSPAPER BODY BELOW THE LOCKED OFFICIAL MASTHEAD. Do not generate any masthead, publication logo, header, publisher mark, or top branding. Create an exact L-BAND advertisement: a RIGHT VERTICAL LEG 15.5 cm wide running through the full 45 cm body height, joined seamlessly to a BOTTOM HORIZONTAL LEG 32.9 cm wide × 15.5 cm high. The two legs must form one continuous L-shaped creative with the selected direction. Keep realistic dense Kannada editorial articles and photographs only in the remaining upper-left 17.4 cm wide × 29.5 cm high rectangle. The ad must touch the right and bottom canvas edges with no outer gutter. Do not place advertising inside the editorial rectangle and do not place editorial content inside the L-band."
    else:
        size = "1024x1536"
